...
public class MybatisMapperMethod {
        ...
        switch (command.getType()) {
            ...
            case SELECT:
                if (method.returnsVoid() && method.hasResultHandler()) {
                     ...
                     */
                    if (org.xiangqian.auto.deploy.util.List.class.isAssignableFrom(method.getReturnType())) {
    ...
    private <T> Object executeForList(SqlSession sqlSession, Object[] args) {
        org.xiangqian.auto.deploy.util.List<T> list = null;
        for (Object arg : args) {
            if (arg instanceof org.xiangqian.auto.deploy.util.List) {
                list = (org.xiangqian.auto.deploy.util.List<T>) arg;
                break;
            }
        }
        ...
        Integer rows = null;
        if (list == null
                || (offset = list.getOffset()) == null || offset.intValue() <= 0
                || (rows = list.getRows()) == null || rows.intValue() <= 0) {
            return list;
        }

        List<T> data = sqlSession.selectList(command.getName(), method.convertArgsToSqlCommandParam(args));
        int size = data.size();
        int maxOffset = offset + size / rows - 1;
        if (size % rows != 0) {
            maxOffset += 1;
        }
        ...
        while (offset0 <= maxOffset) {
            offsets.add(offset0);
            offset0++;
        }

        list.setOffsets(offsets);

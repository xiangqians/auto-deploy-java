...
(
    ...
    `add_time`      INTEGER      DEFAULT 0,            -- 创建时间（时间戳，s）
    `pull_stime`    INTEGER      DEFAULT 0,            -- 【拉取】开始时间（时间戳，s）
    `pull_etime`    INTEGER      DEFAULT 0,            -- 【拉取】结束时间（时间戳，s）
    ...
    `pack_stime`    INTEGER      DEFAULT 0,            -- 【压缩】开始时间（时间戳，s）
    `pack_etime`    INTEGER      DEFAULT 0,            -- 【压缩】结束时间（时间戳，s）
    `pack_msg`      TEXT         DEFAULT '',           -- 【压缩】信息
    `pack_state`    TINYINT      DEFAULT 0,            -- 【压缩】状态，0-待压缩，1-压缩中，2-压缩成功，3-压缩失败
    `upload_stime`  INTEGER      DEFAULT 0,            -- 【上传】开始时间（时间戳，s）
    `upload_etime`  INTEGER      DEFAULT 0,            -- 【上传】结束时间（时间戳，s）
    `upload_msg`    TEXT         DEFAULT '',           -- 【上传】信息
    `upload_state`  TINYINT      DEFAULT 0,            -- 【上传】状态，0-待上传，1-上传中，2-上传成功，3-上传失败
    `unpack_stime`  INTEGER      DEFAULT 0,            -- 【解压】开始时间（时间戳，s）
    `unpack_etime`  INTEGER      DEFAULT 0,            -- 【解压】结束时间（时间戳，s）
